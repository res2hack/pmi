...
<a href="{{ route('page_index')}}"><u>Halaman</u></a>
<i class="fas fa-angle-right mx-2"></i>
...
<form method="post" action="{{ route('page_update', $page->id)}}" enctype="multipart/form-data">
@csrf

<input type="hidden" name="idHalaman" value="{{ $page->id }}">
    <div class="row">
        <div class="col-md-8">
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <input type="text" id="nmJudul" name="nmJudul" class="form-control h-45 border-form bg-form text-dark font-weight-500" 
                            value="{{ $page->title }} @if ($errors->has('nmJudul')){{ old('nmJudul') }}@endif"
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="nmJenisKategori" name="nmJenisKategori" value="{{ $page->slug }}"
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="summernote" name="nmKonten" class="summernote">{{ $page->content }}</textarea>
                    </div>
                    <div class="form-group w-100" id="featuredImage" style="cursor:pointer !important;">
                        ...
                        <div id="image-preview" class="image-preview w-100" @if($page->img_featured)  style="background-image: url('{{ url($page->img_featured) }}');" @endif>
                            <label for="image-upload" id="image-label" >Pilih Gambar</label>
                            <input type="file" name="featuredImage" id="image-upload" class="img-fluid border-form"  />
                        </div>
                        @if($page->img_featured)
                            <div class="mt-2">
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <input type="text" id="metaTitle" name="metaTitle" class="form-control h-45 border-form bg-form font-weight-500"  
                            value="{{ $page->meta_title }}"   onChange="$('#jmlTitle').html($('#metaTitle').val().length);">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="metaDescription" name="metaDescription" class="form-control h-45 border-form bg-form font-weight-500" rows="5" 
                            onChange="$('#jmlDesc').html($('#metaDescription').val().length);">{{ $page->meta_description }}</textarea>
        ...
        <div class="col-md-4">
            @if($page->delete_date)
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <a href="" onclick="$('#idRestore').val({{ $page->id }});" title="Kembalikan Halaman" 
                                    data-toggle="modal" data-target="#modalRestore" class="font-12 btn btn-lg btn-dark w-100">
                                    <i class="fas fa-times mr-2"></i>Restore
                                </a>
                            </div>
                            <div class="col-md-6">
                                <a href="" onclick="$('#idDelete').val({{ $page->id }});" title="Hapus Permanen Halaman" 
            ...
            @else
                @if($page->status === "terbit")
                <a class="card shadow font-weight-bold font-18" 
                    href="{{ route('page_show', $page->slug ) }}" target="_blank" >
                ...
                @else
                <a class="card shadow font-weight-bold font-18" 
                    href="{{ route('page_preview', $page->id) }}" target="_blank" >
                    <div class="card-body text-center">
            ...
            <div class="card shadow border-top5 border-form">
                <div class="card-body">
                    <div class="form-group w-100">
                        ...
                        <div id="image-preview2" class="image-preview w-100" @if($page->img_thumbnail) style="background-image: url('{{ url($page->img_thumbnail) }}');background-repeat: no-repeat;background-size: cover;" @endif>
                    ...
                    <div class="form-group">
                        ...
                        <select name="nmStatus" id="nmStatus" class="form-control selectric" style="font-size:16px;" >
                            ...
                            <option value="terbit" @if($page->status === "terbit") selected @endif>Terbit</option>
                        </select>
                        <input type="hidden" name="nmTerbitDate" value="{{ $page->published_date }}">
                    </div>
