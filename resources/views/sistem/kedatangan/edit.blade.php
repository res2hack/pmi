...
<a href="{{ route('kedatangan_index')}}"><u>Kedatangan</u></a>
<i class="fas fa-angle-right mx-2"></i>
<a href="{{ route('kedatangan_detail', $kedatangan->id)}}"><u>{{ $kedatangan->nama_tki }}</u></a>
<i class="fas fa-angle-right mx-2"></i>
...
<form method="post" action="{{ route('kedatangan_update')}}" enctype="multipart/form-data">
@csrf

<input type="hidden" name="nmStatus" value="edit">
<input type="hidden" name="nmID" value="{{ $kedatangan->id }}">
        ...
        <div class="row mt-3">
            <div class="col-md-6 border-right border-form">
                <div class="form-group row">
                    ...
                    <div class="col-md-5">
                        <input type="date" id="nmTglDatang" name="nmTglDatang" value="{{ $kedatangan->tgl_datang }}"
                            class="form-control h-45 border-form bg-form font-weight-500">
                        <small class="text-danger">{{ $errors->first('nmTglDatang') }}</small>
                    </div>
                    <div class="col-md-4">
                        <input type="time" id="nmJamDatang" name="nmJamDatang" value="{{ $kedatangan->jam_datang }}"
                            class="form-control h-45 border-form bg-form font-weight-500">
                    ...
                    <div class="col-md-9">
                        <select name="nmPesawat" id="nmPesawat" class="form-control select2 w-100">
                            ...
                            @foreach ($pesawat as $pes)
                                <option value="{{ $pes->jenis_id }}" @if($kedatangan->pesawat == $pes->jenis_id) selected @endif>
                                    [{{ $pes->kode }}] - {{ $pes->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group row pt-2">
                    ...
                    <div class="col-md-9">
                        <input type="text" id="nmPaspor" name="nmPaspor" value="{{ $kedatangan->no_paspor }}"
                            class="form-control h-45 border-form bg-form font-weight-500">
                    ...
                    <div class="col-md-9">
                        <select name="nmImigrasi" id="nmImigrasi" class="form-control select2 w-100">
                            ...
                            <option value="{{ $imi->jenis_id }}" 
                                @if($kedatangan->kantor_imigrasi == $imi->jenis_id) selected @endif>{{ $imi->name }}
                            </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-9">
                        <input type="text" id="nmTKI" name="nmTKI" value="{{ $kedatangan->nama_tki }}"
                            class="form-control h-45 border-form bg-form font-weight-500">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-5">
                        <input type="date" id="nmTglLahir" name="nmTglLahir" value="{{ $kedatangan->tgl_lahir }}"
                            class="form-control h-45 border-form bg-form font-weight-500">
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-md-9 mt-1">
                        <div class="custom-control custom-radio custom-control-inline">
                            ...
                            @if($kedatangan->jk === "L") checked @endif >
                        ...
                        <div class="custom-control custom-radio custom-control-inline">
                            ...
                            @if($kedatangan->jk === "P") checked @endif >
                        ...
                        <select name="nmPendidikan" id="nmPendidikan" class="form-control select2 w-100">
                            ...
                            @foreach ($pendidikan as $pdk)
                                <option value="{{ $pdk->jenis_id }}" 
                                    @if($kedatangan->pendidikan == $pdk->jenis_id) selected @endif>{{ $pdk->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                        ...
                        <select name="nmProvinsi" id="nmProvinsi" onchange="getKota();" class="form-control select2 w-100">
                            ...
                            @foreach ($provinsi as $prov)
                                <option value="{{ $prov->id }}" 
                                    @if($kedatangan->provinsi == $prov->id) selected @endif>{{ $prov->nama }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="form-group row pt-2">
                    ...
                    <div class="col-md-9">
                        ...
                        class="form-control border-form bg-form">{{ $kedatangan->alamat }}</textarea>
                    </div>
                </div>
            </div>
            <div class="col-md-6">
                    ...
                    <div class="col-md-9">
                        <select name="nmPptkis" id="nmPptkis" class="form-control select2 w-100">
                            ...
                            @foreach ($perusahaan as $prsh)
                                <option value="{{ $prsh->id }}" @if($kedatangan->pptkis == $prsh->id) selected @endif>
                                    {{ $prsh->nama }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-9">
                        <input type="text" id="nmAgency" name="nmAgency" value="{{ $kedatangan->agency }}"
                        ...
                        <select name="nmNegara" id="nmNegara" class="form-control select2 w-100">
                            ...
                            @foreach ($negara as $neg)
                                <option value="{{ $neg->jenis_id }}" 
                                    @if($kedatangan->negara == $neg->jenis_id) selected @endif>{{ $neg->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                        ...
                        <select name="nmSektor" id="nmSektor" class="form-control select2 w-100" onchange="getPekerjaan();">
                            ...
                            @foreach ($sektor as $sek)
                                <option value="{{ $sek->jenis_id }}" 
                                    @if($kedatangan->sektor == $sek->jenis_id) selected @endif>{{ $sek->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-5">
                        <input type="date" id="nmTglBerangkat" name="nmTglBerangkat" onchange="masaKerja();"
                            class="form-control h-45 border-form bg-form font-weight-500" value="{{ $kedatangan->tgl_berangkat }}">
                    </div>
                    <div class="col-md-4">
                        <div class="input-group mb-2">
                            <input type="text" class="form-control bg-form border-form h-42 text-center"
                                id="nmMasaKerja" name="nmMasaKerja" value="{{ $kedatangan->masa_kerja }}" readonly>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-5">
                        <select name="nmJenisPulang" id="nmJenisPulang" class="form-control select2 w-100">
                            @foreach ($jenis_pulang as $jpul)
                                <option value="{{ $jpul->jenis_id }}" @if($kedatangan->jenis_pulang == $jpul->jenis_id) selected @endif>
                                    {{ $jpul->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4">
                        <div class="input-group mb-2">
                            <input type="text" class="form-control bg-form border-form h-42 text-center" 
                            value="{{ $kedatangan->hari }}" name="nmPulangHari" >
                            <div class="input-group-append h-42 ">
                    ...
                    <div class="col-md-9">
                        <select name="nmMasalah[]" id="nmMasalah" class="form-control select2 w-100" multiple="multiple">
                            ...
                            @foreach ($masalah as $mas)
                                    <option value="{{ $mas->jenis_id }}" @if (in_array($mas->jenis_id, $t_masalah)) selected @endif>{{ $mas->name }}</option>
                        ...
                        <div>
                            ...
                            class="form-control bg-form border-form h-42"  value="{{ $kedatangan->masalah_lainnya }}">
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-md-9">
                        <div>
                            <select name="nmKepulangan" id="nmKepulangan"  onchange="cekKepulangan();"
                                ...
                                @foreach ($pulang as $pul)
                                    <option value="{{ $pul->jenis_id }}"  
                                        @if($kedatangan->kepulangan == $pul->jenis_id) selected @endif>
                                        {{ $pul->name }}</option>
                                @endforeach
                            </select>
                        </div>
                            ...
                            <select name="nmDijemput" id="nmDijemput" class="form-control select2 w-100">
                                @foreach ($dijemput as $jemput)
                                    <option value="{{ $jemput->jenis_id }}"  
                                        @if($kedatangan->dijemput == $jemput->jenis_id) selected @endif>
                                        {{ $jemput->name }}</option>
                                @endforeach
                            </select>
                        </div>
                            ...
                            <select name="nmPulangSendiri" id="nmPulangSendiri" class="form-control select2 w-100">
                                ...
                                @foreach ($pulang_sendiri as $pulsd)
                                    <option value="{{ $pulsd->jenis_id }}" 
                                        @if($kedatangan->pulang_sendiri == $pulsd->jenis_id) selected @endif>{{ $pulsd->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div id="nmNamaPenjemput" class="mt-3" style="display:none;">
                            <input type="text" id="nmDijemputOleh" name="nmDijemputOleh" 
                                class="form-control h-45 border-form bg-form font-weight-500 " 
                                value="{{ $kedatangan->dijemput_oleh }}" placeholder="Nama Penjemput...">
                        </div>
                        <div id="Menggunakan" class="mt-3" style="display:none;">
                            <input type="text" id="nmMenggunakan" name="nmMenggunakan" value="{{ $kedatangan->menggunakan }}" 
                        ...
                        <div id="Transit" class="mt-3" style="display:none;">
                            <input type="text" id="nmTransit" name="nmTransit" value="{{ $kedatangan->transit_kantor }}"
...
@include('sistem.kedatangan.script-edit')
