...
<a href="{{ route('provinsi_index')}}"><u>Provinsi</u></a>
<i class="fas fa-angle-right mx-2"></i>
...
<div class="row">
    <div class="col-md-4">
        <form method="POST" action="{{ route('provinsi_update', $provinsi->id)}}">
            @csrf
            <input type="hidden" name="nmStatus" value="edit">
            <input type="hidden" name="nmID" value="{{ $provinsi->id }}">
            <div class="card shadow border-top5 border-form ">
                <div class="card-body mt-0 pb-1 ">

                    <div class="form-group">
                        <div class="font-weight-bold font-16 mb-2 text-dark">Nama Provinsi</div>
                        <input type="text"  id="nmProvinsi" name="nmProvinsi" value="{{ $provinsi->nama }}"
                                class="form-control bg-form h-45 font-weight-500 border-form" required>
                        <small class="text-danger">{{ $errors->first('nmProvinsi') }}</small>
                    </div>
                </div>
...
@section('script')
    @include('sistem.master.provinsi.table')
@endsection